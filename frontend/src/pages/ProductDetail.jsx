  ...
  return productData ? (
    <div >
        <div className=' w-[100vw] h-[130vh] md:h-[100vh] bg-gradient-to-l from-[#141414] to-[#0c2025] flex items-center justify-start flex-col lg:flex-row gap-[20px]'>
            <div className='lg:w-[50vw] md:w-[90vw] lg:h-[90vh] h-[50vh] mt-[50px] flex items-center justify-center md:gap-[10px] gap-[30px] flex-col-reverse lg:flex-row'>
                <div className='lg:w-[20%] md:w-[80%] h-[10%] lg:h-[80%] flex items-center justify-center gap-[50px] lg:gap-[20px] lg:flex-col flex-wrap '>
                    <div className='md:w-[70px]  w-[40px] h-[40px] md:h-[80px] bg-slate-300 border-[1px] border-[#80808049] rounded-md'>
            ...
            <div className='lg:w-[60vw] w-[100vw] lg:h-[75vh] h-[40vh] lg:mt-[70px] flex items-start justify-start flex-col py-[10px] px-[20px] md:pb-[20px] md:pl-[20px] lg:pl-[0px] lg:px-[0px] lg:py-[0px] gap-[]'>
                <h1 className='text-[30px] font-semibold text-[aliceblue]'>{productData.name.toUpperCase()}</h1>
                <div className='flex items-center gap-1 '>
                    <FaStar className='text-[15px] fill-[#FFD700]' />
                    <FaStar className='text-[15px] fill-[#FFD700]' />
                    <FaStar className='text-[15px] fill-[#FFD700]' />
                    <FaStar className='text-[15px] fill-[#FFD700]' />
                    <FaStarHalfAlt className='text-[15px] fill-[#FFD700]' />
                    <p className='text-[12px] font-semibold pl-[0px] text-[white]'>(124)</p>
                </div>
                <p className='text-[25px] font-semibold pl-[5px] text-[white] my-[10px]'>{currency} {productData.price}</p>

                <p className=' w-[80%] md:w-[60%] text-[15px] font-semibold pl-[5px] text-[white]'>{productData.description} and Stylish, breathable cotton shirt with a modern slim fit. Easy to wash, super comfortable, and designed for effortless style.</p>
                <div className='flex flex-col my-[10px] '>
                    <p className='text-[22px] font-semibold pl-[5px] text-[white]'>Select Size</p>
          <div className='flex gap-2'>
            {
              productData.sizes.map((item, index) => (
                <button key={index} className={`border py-1.5 px-3.5 bg-slate-300 rounded-md 
                  ${item === size ? 'bg-black text-[#2f97f1] text-[15px]' : ''}`} onClick={() => setSize(item)}  >{item}</button>
              ))
            }
          </div>
           <button className='text-[12px] active:bg-slate-500 cursor-pointer bg-[#495b61c9] py-[5px] px-[10px] rounded-2xl mt-[10px] border-[1px] border-[#80808049] text-white shadow-md shadow-black' onClick={()=>addtoCart(productData._id , size)} >{loading? <Loading/> : "Add to Cart"}</button>
                </div>
            <div className='w-[90%] h-[1px] bg-slate-700 mt-[8px]'></div>
            <div className='w-[80%] text-[12px] text-white mt-[10px]'>
